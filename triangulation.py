...
############ FOSHAN ############
def get_camera_parameters_foshan(camera_data, subject, action, nviews):

    K, R, t, dist = {}, {}, {}, {}

    cam_dict = camera_data["extrinsics"][subject][action]
    cam_names = sorted(cam_dict.keys())[:nviews]

    for name in cam_names:

        # ===== intrinsic =====
        K[name] = np.array(
            camera_data["intrinsics"][name]["K"]
        ).reshape(3, 3)

        dist[name] = np.array(
            camera_data["intrinsics"][name]["distortion"]
        )

        # ===== extrinsic =====
        cam = cam_dict[name]

        rvec = np.array(cam["R"]).reshape(3,)
        R_mat = Rotation.from_rotvec(rvec).as_matrix()

        t_raw = np.array(cam["T"]).reshape(3, 1)
        tvec = -R_mat @ t_raw 
...
def triangulation(dataset, dataset_loader, output_dir, log):
    ...
    for scene_id, scene_data in dataset_loader:
        ...
        if "panoptic" in dataset.data_root:
            ...
            P = create_projection_matrix(K_cameras, R_cameras, t_cameras)
        if "foshan" in dataset.data_root:
            subject = scene_name.split("_")[0]
            action  = scene_name.split("_")[1]
            
            K_cameras, R_cameras, t_cameras, dist_cameras = get_camera_parameters_foshan(camera_data,subject,action,dataset.nviews)
            P = create_projection_matrix(K_cameras, R_cameras, t_cameras)
