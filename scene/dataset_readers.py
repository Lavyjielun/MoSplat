...
class DataLoader:
    def __init__(self, data_root, initial_guess_dir, poses_2d_dir, frame_step=64, start_id=0, end_id=2181, nviews=4):
        ...
        elif "foshan" in self.data_root:
            metadata_path = os.path.join(self.data_root, "camera_parameter.json")
...
def getFoShanCamera(data, subject, action, camera_name):
    intri = data["intrinsics"][camera_name]
    calibration_matrix = np.array(intri["K"]).reshape(3, 3)

    extri = data["extrinsics"][subject][action][camera_name]

    rvec = np.array(extri["R"]).reshape(3,)
    tvec = np.array(extri["T"]).reshape(3, 1)

    R = Rotation.from_rotvec(rvec).as_matrix()
    T = -R @ tvec 
    width, height = 512, 512   # 你这个数据是512
...
def readCamerasFromTransforms(path, transformsfile, depths_folder, white_background, is_test, extension=".png"):
    ...
    return cam_infos
def readFoShanSceneInfo(path, pose_3d, cameras, scene_name):
